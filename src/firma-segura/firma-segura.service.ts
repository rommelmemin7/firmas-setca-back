...
export class FirmaSeguraService {
	...
	async updateApplicationsStatus() {
		const applications = await this.prisma.application.findMany({
			where: {
				externalStatus: {
					in: ['REGISTERED', 'VALIDATING', 'REFUSED', 'ERROR', 'GENERATED', 'EXPIRED', 'PENDING', 'MANUALLY_VALIDATING', 'AUTOMATICALLY_VALIDATING', 'COMPLETED'],
				},
			},
		});
		...
		for (const app of applications) {
			try {
				...
				console.log(`Verificando estado de la solicitud ${JSON.stringify(statusResponse)}...`);
