...
export class Utils {
	...
	static restarFechas(fecha1: Date, fecha2: Date): number {
		...
		const diferenciaMilisegundos = Math.abs(milisegundosFecha1 - milisegundosFecha2);
		return Math.floor(diferenciaMilisegundos / 1000);
	}

	static decryptPassword(encryptedPassword: string, key: string, ivString: string): string {
	...
	static encryptPassword(password: string, key: string, ivString: string): string {
		const iv = Buffer.from(ivString, 'utf8');
	...
	static formatDates<T extends { createdAt: Date; updatedAt: Date; approvedAt?: Date | null }>(data: T | T[]): any {
		const formatItem = (item: T) => ({
			...item,
			createdAt: item.createdAt.toISOString(),
			updatedAt: item.updatedAt.toISOString(),
			...(item.approvedAt ? { approvedAt: item.approvedAt.toISOString() } : { approvedAt: null }),
		});
	...
	static calculateTotalWithIVA(amount: number, desc: number, ivaPercentage: number): number {
